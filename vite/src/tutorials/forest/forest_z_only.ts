...
import THIS_URL from './forest_z_only.ts?url';
import { TutorialApplication } from "../common/tutorial/tutorial";
import { PerfLogger } from "../common/perf";

const POOL_SIZE = 8;

export default class ForestZTutorial extends ForestTutorial {
...
/*
const SCALE = 1;
const WIDTH = Math.ceil((1280/SCALE)/16)*16;
...
export async function run(START_TIME: number) {
  ...
  const tutorial = new ForestZTutorial();
  ...
  perf.log('Put Pixels', () => {
    ...
    for (let y = 0; y < HEIGHT; y++) for (let x = 0; x < WIDTH; x++) {
      ...
      pixels.data[index] = pixels.data[index+1] = pixels.data[index+2] = 255-Math.log1p(z)*25;
...
export async function runWithWorker(START_TIME: number) {
  return TutorialApplication.runTutorialWithWorkers(START_TIME, ForestZTutorial, THIS_URL, 'canvas', WIDTH, HEIGHT, 4);
}